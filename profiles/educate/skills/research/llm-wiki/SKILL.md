...
- `references/shareable-guide-workflow.md` — how to create self-contained, shareable guides from wiki/vault conventions (privacy stripping, agent-droppable structure, verification)
...
### Creating Pages from Gathered Context (Non-Ingest)

When the user asks to "write up," "document," or "review and write in the wiki" something that exists on disk (a profile, a setup, a system) rather than providing a source document, see `references/wiki-page-creation-workflow.md` for the parallel-gather → existing-check → write → index+log workflow.
...
## Health Check (merged from `wiki-health-check`)

Run periodic health checks on the LLM Wiki to catch structural issues before they compound. Use as a daily/weekly cron job or after major content additions.

**Checks:** broken wikilinks, orphan pages, stubs, duplicate content, tag drift, source SHA256 drift, page size (>200 lines = split candidate), log rotation (>500 entries).

**Lint script** (in this skill's `scripts/` directory):
```bash
WIKI_PATH="${WIKI_PATH:-~/wiki}"
python3 "$WIKI_PATH/scripts/wiki-lint.py" "$WIKI_PATH"
```

**Fix workflow:** Run lint → for each issue category, apply the fix (create missing pages, add inbound links, expand stubs, merge duplicates, update tags, recompute SHA256, plan splits) → re-run lint → update index.md and log.md.

**Runner script:** `scripts/runner.sh` — wraps the lint script for cron execution.
