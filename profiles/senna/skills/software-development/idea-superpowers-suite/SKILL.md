...
IDENTITY: IdeaPipeline{CaptureToBuildHandoff,FullOrLite}. CoreRole: Convert rough ideas into structured artifact chains: capture → design → optional UI brief → research → implementation spec → build handoff → spec review. BehavioralContract: Lite mode stays fast. Full mode gates each stage. GREENLIGHT NEXT STAGE overrides.
Law: Never move from design to implementation if open product/UI questions would change architecture or scope.
WHENUSE: User wants full multi-step idea workflow, or a structured concepting pass before building a creative/visual project. ESPECIALLY:{NewProductIdea,CreativeSceneConcept,AppSpecNeeded,BuildHandoffForAgent,ProjectReboot}. NoSkip:{ClarifyingInterview,ArtifactPerStage}.
REDFLAGS: UIStageMandatory->OptionalOnly|ImagesAsSpec->TranslateToText|InconsistentRenumbering->PreserveExisting|OverloadingLite->KeepFast|GenericUISlop->ProductSpecific|CodeBeforeProductClarity->WrongOrder.
RATIONALIZATIONS: "It's just a note"->UseLiteMode|"Skip the interview"->CaptureFirst|"I know what I want"->DesignDocExposesGaps.
QUICKREF: Capture{title,summary,problem,mode}->Interview{one question at a time}->DesignDoc{product+tech shape}->UIBrief{optional Full only}->Research{similar products}->ImplSpec{stack,tasks,tests}->BuildHandoff{agent-ready}->SpecReview{PASS|PASSWTIHCHANGES|FAIL}.
...
Turn a rough thought into a structured chain of artifacts. Handles both:

- **Product/app ideas** — traditional product-to-build pipeline (Full mode)
- **Creative/visual scene concepts** — cinematic sequences, 3D scenes, visual experiences (Lite mode, focus on mood/aesthetic/technical shape)
- **Project reboots** — when the user scraps an existing project and wants to start fresh with a completely different concept. The Capture stage should first acknowledge the discarded work, then proceed with the new concept as a clean slate. Document the pivot explicitly in the concept doc so scope is clear.
...
**For creative/visual scene concepts** (cinematic sequences, 3D scenes), tailor questions to:
- Setting and mood (time of day, weather, atmosphere)
- Camera POV and shot sequence (acts/scenes)
- Visual palette (colors, lighting style, texture level)
- Subject design (characters, creatures, vehicles) — complexity + detail level
- Duration and pacing
- What makes it feel excellent — the "wow" moment

For Full mode (products/apps), cover:
...
7. **Handling a project reboot without confusion** — when the user completely changes scope ("scrap what we have, start fresh"), the skill should:
   - Acknowledge the discarded work explicitly (don't silently drop it)
   - Treat the new concept as a clean capture with its own design doc
   - Kill any running dev servers/services from the old project
   - Mark the old project state clearly in memory so future sessions don't try to resume the wrong thing
   - Use Lite mode for the new concept unless the user asks for Full mode — creative pivots benefit from fast capture, not heavy process
