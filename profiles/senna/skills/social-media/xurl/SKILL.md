...
**Recommended:** the shell script. Homebrew cask install can timeout (120s+) on macOS, especially on first tap.

```bash
# Shell script (installs to ~/.local/bin, no sudo, works on Linux + macOS) ← PREFERRED
curl -fsSL https://raw.githubusercontent.com/xdevplatform/xurl/main/install.sh | bash

# Homebrew (macOS) — may timeout on first run due to tap cloning
...
1. Create a developer account and app at https://console.x.com.
   ...
   - ⚠️ **"Website URL"** is a required field — it doesn't affect functionality, just put any valid URL (e.g. your GitHub profile or `https://example.com`).
...
### Bookmarks extraction (filter by domain)
```bash
# Fetch bookmarks, filter for specific domains
xurl bookmarks -n 100 | jq -r '.data[] | select(.text | test("github\\.com"; "i")) | .text' | grep -oE 'https://github\.com/[^\s)]+' | sort -u

# Alternative: grab all bookmarks as JSON for downstream processing
xurl bookmarks -n 100 > /tmp/bookmarks.json
```
Bookmarks are private — auth required. Rate limit: ~50 requests per 15min window on free tier.
...
- **Do NOT create `~/.xurl/` as a directory.** xurl expects `~/.xurl` to be its own YAML config file. If you create a directory there (e.g. to stash a `.env` with creds), xurl fails with `"IO Error: open ~/.xurl: is a directory"`. Use a different path for any credential staging (e.g. `/tmp/xurl-creds.env`), or have the user paste values directly into the `xurl auth apps add` command.
- **Most endpoints require paid API credits.** Bookmarks, timeline, mentions, search — nearly everything beyond basic auth checks is gated. If `xurl auth status` works but data endpoints return `CreditsDepleted`, the user needs to add credits in the Developer Console (Billing → minimum $5) or use a pay-per-use plan. Free tier is essentially auth-only.
- **brew cask install can be slow/timing out.** The `brew install --cask xdevplatform/tap/xurl` method pulled a full Homebrew auto-update first and timed out at 120s. The shell script `curl -fsSL https://raw.githubusercontent.com/xdevplatform/xurl/main/install.sh | bash` installed in seconds. Prefer the shell script on macOS when brew is slow.

## Troubleshooting

| Symptom | Cause | Fix |
| --- | --- | --- |
| `IO Error: open ~/.xurl: is a directory` | A directory was created at `~/.xurl/` (e.g. for staging creds) — xurl expects this path for its own YAML config | Move/remove the directory: `rm -rf ~/.xurl`, then re-run `xurl auth apps add` |
| `CreditsDepleted` on any data endpoint | $0 balance on X API | Add credits in Developer Console → Billing (min $5), or upgrade to a paid plan |
...
| `brew install --cask` hangs/times out | First tap clone is slow on some networks | Use the shell script installer instead: `curl -fsSL https://raw.githubusercontent.com/xdevplatform/xurl/main/install.sh \| bash` |
