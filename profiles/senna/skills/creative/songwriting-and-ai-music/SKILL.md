...
## 10. Audio Analysis with songsee

For analyzing audio files (spectrograms, feature extraction), use the `songsee` CLI tool:
...
```bash
songsee track.mp3 --viz spectrogram,mel,chroma,hpss -o analysis.png
songsee track.mp3 --start 12.5 --duration 8 -o slice.jpg
```

Output images can be inspected with `vision_analyze` for automated audio analysis.

## 11. Local Music Generation with HeartMuLa

HeartMuLa is an open-source music foundation model (Apache-2.0) that generates full songs from lyrics + tags. Comparable to Suno but runs locally.

**Hardware:** 8GB VRAM minimum (with `--lazy_load true`), 16GB+ recommended
...
**Requires source patches** for transformers 5.x compatibility — see the archived `heartmula` skill for full patch instructions (RoPE cache fix + HeartCodec loading fix).
...
**Tags:** comma-separated genre/mood/instrument descriptors (e.g., `piano,happy,wedding,synthesizer`)
**Lyrics:** use `[Intro]`, `[Verse]`, `[Chorus]`, `[Bridge]`, `[Outro]` structural tags
**Performance:** RTF ≈ 1.0 — 4-minute song takes ~4 minutes. Output: MP3, 48kHz stereo.

**Key pitfalls:** Don't use bf16 for HeartCodec (degrades quality). Tags may be ignored (#90). Triton = Linux/CUDA only.
