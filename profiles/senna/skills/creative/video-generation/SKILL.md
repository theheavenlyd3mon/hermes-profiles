...
1. text_to_video 15s carrying SHOT ONE ... HARD CUT ... SHOT TWO (~7-8s each).
2. video_continuation 15s from that file carrying the final shot (open with "Continue this video from its final frames:", re-state the character description verbatim).
3. Stitch losslessly:
...
4. RHYTHM: one job at a time means the blocking get_result poll IS your cooldown — the moment a segment returns Ready, submit the next one, and run the ffmpeg stitch in the same batch (they're independent). Never batch multiple submits; all but the first fail.
5. Lock the character description + style prefix verbatim across every segment; the continuation inherits motion but not identity.
