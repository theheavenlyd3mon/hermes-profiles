...
**Likely targets:**
- `~/.hermes/.env` — root, shared across profiles (~186 keys — the primary threat). This is the canonical source of API keys, tokens, and paths. **Always use absolute paths** (`/Users/$USER/.hermes/.env`) — `test -f ~/.hermes/.env` can report false negatives in the terminal tool.
- `~/.hermes/profiles/<name>/.env` — profile-specific overrides (1-4 keys typically). These either add provider-specific keys (e.g., Xiaomi MiMo) or redundantly copy root keys (e.g., `OPENROUTER_API_KEY`). See `references/env-layering-architecture.md` for the full layering model and redundancy cleanup workflow.
