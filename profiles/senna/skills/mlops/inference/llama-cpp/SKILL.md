...
- **For MoE models on limited VRAM:** use `--cpu-moe` to keep expert weights in system RAM. A 22GB Q4_K_M MoE can run on 12GB VRAM this way. See [constrained-vram-models.md](references/constrained-vram-models.md).
- **For MoE models, prefer APEX quants** (by mudler/LocalAI): MoE-aware mixed-precision that beats Q8_0 at half the size. Tiers: Quality (22GB), Balanced (24GB), Compact (16GB), Mini (13GB), Nano (11GB experimental). I-Mini is the smallest "safe" tier. See [quantization.md](references/quantization.md) for full APEX breakdown.
- **For MTP-capable models:** add `--spec-type draft-mtp --spec-draft-n-max 2` for 1.5-2.7x speedup on code/factual tasks. Requires llama.cpp b9180+. See [advanced-usage.md](references/advanced-usage.md).
- **For EAGLE-3 speculative decoding:** requires SGLang/vLLM (not llama.cpp), separate drafter file, and dense target models. Doesn't fit 12GB VRAM — MoE with MTP is faster on constrained hardware. See [constrained-vram-models.md](references/constrained-vram-models.md) for full analysis.
...
### MoE on limited VRAM (cpu-moe)

```bash
# 22GB MoE model on 12GB VRAM — experts stay in system RAM
llama-server -hf bartowski/Qwen_Qwen3.6-35B-A3B-GGUF:Q4_K_M \
    -ngl 99 --cpu-moe -c 8192 --flash-attn
```

### MoE with tensor offload (fine-grained control)

```bash
# Alternative to --cpu-moe: regex-based tensor offloading
# Keeps attention on GPU (~3-5 GB VRAM), expert FFN layers in system RAM (~16 GB)
# More control than --cpu-moe — you can target specific layer patterns
...
### MoE + MTP speculative decoding (fastest setup)

```bash
# MoE + cpu-moe + MTP for maximum speed on limited VRAM
# NOTE: MTP merged to mainline llama.cpp May 2026 — no fork needed
llama-server -hf bartowski/Qwen_Qwen3.6-35B-A3B-GGUF:Q4_K_M \
    -ngl 99 --cpu-moe \
...
### AtomicBot TurboQuant + AtomicChat UDT (best for 12GB VRAM)

Requires AtomicBot fork. See [atomicbot-turboquant.md](references/atomicbot-turboquant.md) for build instructions and fork comparison.

```bash
# AtomicChat UDT — best quality/speed/size, TurboQuant3 KV cache, NextN MTP
# Enables 128K context on 12GB VRAM (vs 32K with mainline)
# NOTE: do NOT add --cpu-moe for UDT — expert offloading is handled by the quantization mask
llama-server \
...
- **[advanced-usage.md](references/advanced-usage.md)** — MTP speculative decoding (preferred), draft models, batched inference, grammar-constrained generation, LoRA, multi-GPU, custom builds
- **[constrained-vram-models.md](references/constrained-vram-models.md)** — curated models for 12GB VRAM, --cpu-moe recipes, uncensored vs stock vs MTP comparison, download URLs, AtomicBot/AtomicChat UDT setup
- **[atomicbot-turboquant.md](references/atomicbot-turboquant.md)** — AtomicBot fork: TurboQuant KV cache (turbo3/4), NextN speculative decoding, AtomicChat UDT GGUFs, build/run commands for Windows
- **[quantization.md](references/quantization.md)** — quant quality tradeoffs, when to use Q4/Q5/Q6/IQ, model size scaling, imatrix, Unsloth UD variants
- **[glossary.md](references/glossary.md)** — beginner-friendly explanations of GGUF, quantization, MoE, --cpu-moe, MTP, VRAM, KV cache, abliteration, and other local LLM terms
...
- **[optimization.md](references/optimization.md)** — CPU threading, BLAS, GPU offload heuristics, MoE + cpu-moe, batch tuning, benchmarks
