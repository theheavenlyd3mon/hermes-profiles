...
- **unsloth** — Fine-tune abliterated models with less VRAM

## Community Alternative: Heretic

**Heretic** (https://github.com/andyrdt/heretic) is a popular community abliteration tool using MPOA (Magnitude-Preserving Orthogonal Abliteration). Key differences from OBLITERATUS:

- **Two-stage pipeline**: output-side orthogonalization (self_attn.o_proj, mlp.down_proj) then input-side (mlp.gate_proj, mlp.up_proj)
- **Slot-grouped**: groups layers by `layer_index % 4` for targeted processing
- **Norm-preserving**: restores L2 norm after each projection
- **Lower quality loss**: KL divergence ~0.028 on harmless prompts (very good)

Heretic is widely used for Qwen models on HuggingFace. Search for `heretic GGUF` or `abliterated heretic` on the Hub. Popular uploaders: `Youssofal`, `DavidAU`, `HauhauCS`, `llmfan46`.

When the user asks about abliterated models they found on HuggingFace (not running their own abliteration), the Heretic-produced GGUFs are likely what they're looking for.
