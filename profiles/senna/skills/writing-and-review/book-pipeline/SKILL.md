...
**User phrases (hard protocol):**
- **"use the team"** → parallel `delegate_task` to specialists; do **not** solo multi-domain work from the orchestrator.
- **"don't do all the work" / "as needed"** → propose lanes; batch go-ahead when ambiguous; never expand a skill test into a full pipeline build unprompted.
- **"after get all back then report"** → hold partials; one consolidated brief when every lane returns (status table + conflicts + recommendation). Never drip five half-reports.

**Report protocol:** multi-lane fan-out → **wait for all** → **one consolidated brief**. See `references/orchestrator-fanout.md`.

**Locked decisions:** `references/locked-decisions.md` — Darwin-only; stdlib assembler canonical; Kokoro→Findaway. Build lessons: `references/session-lessons-pipeline-build.md`.
...
- **Parallel fan-out duplicates artifacts.** Two CLIs or model briefs → mark CANONICAL vs DEPRECATED immediately; update `locked-decisions.md`. Prefer stdlib assembler unless user asks for covers/print quality.
- **Subagents invent skills / patch SKILL.md unprompted.** After fan-out, audit the skills tree; keep only intentional patches.
- **Scaffold + `str.format`.** YAML braces break Python `.format()`. Use `__PLACEHOLDER__` + `.replace()` in `scripts/init_manuscript.py` — never `{slug}` inside YAML templates.
- **Profile describe CLI:** `hermes profile describe <name> --text "…"` (not `--description`).
- **macOS vs Windows host:** ship `book-writer-pipeline/windows/*` for the your GPU; do not claim Darwin is live until that box starts `llama-server`.
- Expanding a narrative skill *test* into architecture/profile work without being asked.

## Support files
- `references/locked-decisions.md`
- `references/orchestrator-fanout.md`
- `references/style-revision-playbook.md`
- `references/session-lessons-pipeline-build.md` — build pitfalls + smoke sequence
...
- Companion: `narrative/scripts/check_manuscript.py`; `book-writer-pipeline/windows/*`
...
- [ ] After multi-agent design: one canonical CLI, one model story, skills tree audited
