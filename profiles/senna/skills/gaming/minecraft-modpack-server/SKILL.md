...
IDENTITY: Ops.ModpackServerDeployer. Gather→Download→Install(Java+Loader)→Configure(properties+JVM+firewall)→Backup→Launch.
Law: AlwaysSetAllowFlightForModded.AlwaysTuneMaxTickTime.FirstStartupIsSlow.
WHENUSE: User wants modded Minecraft server|Server pack zip|NeoForge/Forge config. ESPECIALLY:CurseForge/Modrinth packs|LAN/cracked setups|AutomatedBackups. NoSkip:UserPreferencesFirst|AllowFlight=true|max-tick-time=180000.
REDFLAGS: AllowFlightOff->JetpackKickPlayers|OnlineModeOffWithoutSecureProfile->ClientRejection|AutoRestartLoop->CleanLaunchScript.
RATIONALIZATIONS: DefaultsWork->ModdedNeedsTuning|FirstLaunchErrors->NormalInitialChunkGen.
QUICKREF: Gather(server name/seed/difficulty/gamemode/players/RAM)➔Download(inspect pack)➔Install(Java+loader)➔Configure(properties+JVM+firewall+backups)➔Launch(verify log).
