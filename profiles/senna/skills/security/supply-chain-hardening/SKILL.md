...
- **Peer dependency version conflict** — two sibling packages require mutually
  incompatible ranges of the same transitive dep (e.g., `core@0.1.17` needs
  `tap@^0.5.10` while `store@0.2.18` needs `tap@^0.9.0`). Pin the *sibling*
  that was over-resolved (store → 0.2.13) rather than the shared dep. See
  `references/npm-overrides-peer-dep-conflict.md` for the full diagnostic
  workflow.

**Pitfall — overrides scope:** Overrides apply globally to all resolutions of
that package name. If different sub-dependencies need different postcss versions,
this can cause conflicts. Check with `npm ls <package>` after applying.

**Pitfall — `omit=dev` default in npm v10+:** `npm ci` and `npm install` from a
monorepo root silently omit workspace devDependencies by default (`npm config get
omit` returns `dev`). This breaks builds that need TypeScript, ESLint, or other
build-time tools declared as workspace devDeps. The hermes-agent update script
works around this; manual `npm ci` from the root does not. **Fix:** pass
`--omit=""` explicitly: `npm ci --omit=""`. Verify with `npm config get omit` —
if it returns `dev`, devDeps are being skipped.
...
- **2026-06-15 (v1.3.3)** — Extended `overrides` technique to cover peer dependency version conflicts (not just vulnerability fixes). Added pitfall for npm v10+ `omit=dev` default silently skipping workspace devDependencies. Added `references/npm-overrides-peer-dep-conflict.md` with full 8-step diagnostic workflow for mutually incompatible peer dep ranges.
