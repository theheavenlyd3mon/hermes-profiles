...
# Or for selective skill seeding:
hermes profile create <name> --no-skills
```

**`--no-skills` does NOT create an empty profile.** It still installs ~20-30 bundled category directories (apple, creative, software-development, etc.) with `DESCRIPTION.md` files but no actual skill content. This is the expected starting state for selective seeding. Don't treat it as an error.
...
### Role-to-toolset mapping (17-profile fleet, June 2026)

**Discord bots (8):**
| Profile | Role | CLI Tools |
|---------|------|-----------|
| senna | Coordinator | (full — all tools) |
| code | Implementation | file, terminal, skills, web, vision, session_search |
| creative | Design & visual art | file, terminal, skills, web, vision, session_search, image_gen, browser |
| research | Investigation | file, terminal, skills, web, vision, session_search, browser, fabric, memory |
| finance | Trading & market | file, terminal, skills, web, vision, session_search, browser, fabric, memory |
| knowledge | Docs & wiki | file, terminal, skills, web, memory, session_search, messaging, vision, fabric |
| infra | DevOps | file, terminal, skills, web, vision, session_search |
| security | Security audits | file, terminal, skills, web, vision, session_search |

**Kanban workers (9):** All get `file, terminal, skills, web, vision, session_search` unless noted.

**Rationale for key decisions:**
- `messaging` — only senna (routing) and knowledge (pushing docs). Workers respond when called.
- `memory` — only senna (project state), research (watchlists), finance (trade history), knowledge (knowledge base).
- `fabric` — only research (content extraction), finance (market analysis), knowledge (document processing).
- `browser` — only creative (design references), research/finance (navigating live sites).
- `image_gen` — only creative.
- `delegation` — only senna (foreman merged into senna).
- `kanban` — only senna.
...
|--------------|-------------------|-----------|
| **senna** (user chat + orchestrator) | ✅ YES | Primary interface + merged foreman — needs delegation for quick one-shots + Kanban for durable work |
| **All workers** (code, creative, research, finance, knowledge, infra, security, ue5, mlops, cyber-*, business, media, homelab, social, communication) | ❌ NO | Workers should never spawn children. Multi-profile work goes through Kanban. |
...
**Pitfall: `hermes profile create` creates SOUL.md and seeds builtin skills, but NOT config.yaml or .env.** The create command gives you a profile directory with SOUL.md and category directories under `skills/` (containing builtin skill stubs), but config.yaml and .env must be written manually. If you run `hermes profile list` and see the profile with a default model, that's Hermes' fallback — not a real config. Always write config.yaml explicitly.

**Pitfall: No root `auth.json` may exist.** If profiles were recreated from scratch, `~/.hermes/auth.json` may be gone. Hermes stores API keys in an internal secrets store (visible via `hermes config show`). Profiles can work without auth.json if the keys are in the secrets store. Don't block on auth.json symlinks — verify with a smoke test instead.

**Pattern: Writing config.yaml for 17+ profiles.** When standing up many profiles at once, write a Python script that generates all config.yaml files from a profile→attributes dict. Much faster and more consistent than `hermes config set` per profile (which also has the "writes to active profile" pitfall). See `references/batch-config-generation.md`.

**Pitfall: `.env` files should contain ONLY secrets, not URLs or behavioral config.** The `base_url` for non-standard providers (e.g., Xiaomi's `https://token-plan-sgp.xiaomimimo.com/v1`) belongs in config.yaml's `model.base_url`, NOT in .env. `.env` is for API keys and tokens only. Writing URLs to .env creates a second source of truth that drifts from config.yaml.

**Pattern: .env placeholders for deferred key entry.** When setting up profiles before the user has API keys ready, write `.env` files with `REPLACE_ME` placeholders and clear comments showing what each key is for. Group by provider (Xiaomi block, OpenRouter block, Discord block). This lets the user fill in keys at their desk without guessing format.

**Pitfall: Magnus skills filename typos.** The `opensource-contributions` skill in the Magnus repo is spelled correctly — but the skill-to-profile-mapping doc had it as `opensouce-contributions` (missing 'r'). Always `ls` the actual repo directory before writing copy commands. See `references/magnus-skill-installation.md`.
...
**Pitfall: Skills can be nested 2-3 levels deep.** When installing skills from root or other profiles, the actual path may differ from the expected path. For example, `mlops/llama-cpp` is actually at `mlops/inference/llama-cpp/`. Always `find` or `ls` the category directory before writing copy commands. See the `multi-agent-profile-redesign` skill's `references/skill-installation-patterns.md` for the full gotcha table.

**Pitfall: `find` output can be garbled in `execute_code`.** The `find` command's output sometimes gets mangled when run inside `execute_code` scripts — line counts appear as `1F 1D:` prefixes instead of clean paths. Use `terminal()` directly for discovery commands.
...
**As of June 2026:** 17 domain-based profiles, 8 Discord bots. Foreman merged into senna. Old profiles (architect, coder, designer, foreman, oracle, researcher, secretary, data-analyst, debugger, devops, reviewer) deleted.

**Discord bots (8):**

| Profile | Channel | Role | Model |
|---------|---------|------|-------|
| senna | #nexus-hq | Coordinator + orchestrator | xiaomi/mimo-v2.5 |
| code | #engineering | Implementation + debug + review | xiaomi/mimo-v2.5 (high) |
| creative | #design-studio | Design, visual art, media gen | xiaomi/mimo-v2.5 |
| research | #research-lab | Investigation, data, academia | xiaomi/mimo-v2.5 (high) |
| finance | #market-intel | Trading, market analysis | xiaomi/mimo-v2.5 (high) |
| knowledge | #writing-desk | Obsidian, docs, wiki | openrouter/owl-alpha:free |
| infra | #operations | DevOps, deployment, networking | xiaomi/mimo-v2.5 |
| security | #security-ops | Security audits, orchestrates cyber-* | xiaomi/mimo-v2.5 (high) |

**Kanban workers (9):** ue5, mlops, cyber-red, cyber-blue (coordinator), cyber-blue-cloud, cyber-blue-forensics, cyber-blue-compliance, cyber-blue-soc, business, media, homelab, social, communication.

**Cyber profile skill split (June 2026):**
| Profile | Skills | Domain |
|---------|--------|--------|
| cyber-red | ~160 | Offensive: pen testing, red team, malware analysis |
| cyber-blue | 0 | Coordinator → delegates to sub-profiles |
| cyber-blue-cloud | ~123 | Cloud security |
| cyber-blue-forensics | ~35 | Forensics + threat intel |
| cyber-blue-compliance | ~22 | Compliance + IAM |
| cyber-blue-soc | ~371 | SOC + network + IR + vuln mgmt |

Server ID: 1508910747382583610.
