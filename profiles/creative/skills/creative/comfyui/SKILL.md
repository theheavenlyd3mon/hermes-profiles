...
- `video-models-2026.md` — video generation model landscape: Wan 2.2 (5B/14B),
  LTX-Video 2.3, HunyuanVideo, AnimateDiff, cloud APIs, VRAM tiers,
  Blender→I2V pipeline, camera language prompts, post-processing.
...
Video workflows auto-detect via output nodes (`VHS_VideoCombine`, `SaveVideo`)
and bump timeout from 300s → 900s. Override with `--timeout 1800`.

**Best consumer-GPU option: Wan 2.2 5B Hybrid** (~8 GB VRAM, T2V + I2V):
```bash
# Text-to-video
python3 scripts/run_workflow.py \
  --workflow workflows/wan_video_t2v.json \
  --args '{"prompt": "slow dolly forward through misty forest, cinematic lighting"}' \
  --output-dir ./video-outputs

# Image-to-video (3D render → animated)
python3 scripts/run_workflow.py \
  --workflow workflows/wan_video_t2v.json \
  --input-image image=./blender_render.png \
  --args '{"prompt": "camera slowly orbits, atmospheric particles drift"}' \
  --output-dir ./video-outputs
```
...
**Model selection:** See `references/video-models-2026.md` for full comparison.
Key choice points: Wan 2.2 5B (consumer GPU, 8GB), Wan 2.2 14B (datacenter),
LTX-Video 2.3 (fast, 24GB), HunyuanVideo (best quality, 60-80GB).

**3D→AI pipeline:** Render still in Blender → feed to Wan 2.2 I2V → AI adds
atmosphere, organic motion, particles → optionally composite back in Blender.

**Post-processing:** ComfyUI outputs yuv444p. Re-encode for Discord/browser:
```bash
ffmpeg -y -i input.mp4 -c:v libx264 -profile:v main -crf 13 -pix_fmt yuv420p -c:a aac -b:a 192k output.mp4
```
...
13. **Wan 2.2 MoE dual-pass** — the 14B T2V/I2V models use Mixture of Experts
    with separate high-noise and low-noise model files. Both must be loaded;
    don't try to use just one. The 5B hybrid is a single file.
