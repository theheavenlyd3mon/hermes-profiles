...
**Pitfall — plugin enabled but tools never appear (silent import failure):**
Hermes imports directory plugins as `hermes_plugins.<slug>` WITHOUT adding the
plugin dir to `sys.path`, so third-party plugins with absolute self-imports
die at load with `Failed to load plugin '<name>': No module named '<pkg>'` in
the agent log — config and manifests look fine. Relative-import fix at the top
level alone is insufficient (submodules break too); the working fix is
prepending the plugin dir to sys.path in `__init__.py`. Bit hermes-image-studio
2026-08-05: senna's clone fixed (commit c24b73b), creative profile's separate
clone still carries the bug. Full fix + loader-replay verifier: `hermes-maintenance`
skill §6e + `scripts/plugin-load-test.py`, `references/plugin-import-failure.md`.
