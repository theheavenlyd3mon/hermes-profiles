...
When running inside a profile (e.g. Senna), `$HOME` resolves to `~/.hermes/profiles/senna/home` — NOT `~`. This means `source ~/.hermes/.env` fails. Always use the absolute path:
