...
description: "Kanban multi-agent workflow: orchestrator decomposition playbook, worker lifecycle, task routing, and pitfalls."
version: 3.0.0
...
When you're spawned as a kanban worker (dispatcher assigned you a card), your job is to execute the task and hand off results cleanly. The core lifecycle is auto-injected via `KANBAN_GUIDANCE`; this section covers the deeper patterns.
...
**Review-required coding task:** Block instead of complete. Use `kanban_comment` for structured metadata, then `kanban_block(reason="review-required: ...")`. See `references/worker-guide.md` for the full pattern.
...
Pass created card ids in `created_cards` on `kanban_complete`. The kernel verifies each id exists and was created by your profile. **Never invent ids** — only list ids captured from successful `kanban_create` return values.

```python
c1 = kanban_create(title="remediate SQL injection", assignee="security-worker")
kanban_complete(
    summary="Review done; spawned remediation.",
...
```python
kanban_comment(body="Full context: I have user IPs but some are behind NATs...")
kanban_block(reason="Rate limit key choice: IP (NAP-unsafe) or user_id (requires auth)?")
```
...
- Don't call `delegate_task` as substitute for `kanban_create`
- Don't call `clarify` — there's no live user. Use `kanban_comment` + `kanban_block` instead
- Don't modify files outside `$HERMES_KANBAN_WORKSPACE` unless the task body says to
- Don't create follow-up tasks assigned to yourself
- Don't complete a task you didn't finish — block it instead

For full worker details (retry scenarios, tenant isolation, notification routing, CLI fallbacks), see `references/worker-guide.md`.
