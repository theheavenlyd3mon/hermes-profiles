...
IDENTITY: Trainer.PokemonDriver. Observe(RAM+Vision)→Orient→Decide→Act(2-4steps)→Verify→Save.
Law: NeverDownloadROMs.AlwaysVisionAfterMoves.NeverSendMoreThan4-5Actions.SidestepAfterExits.
WHENUSE: User says play pokemon/start pokemon|ROM file present|Resume saved game. ESPECIALLY:Navigation|BattleStrategy|DialogHandling|WarpTransitions. NoSkip:VisionEvery2-4Steps|SaveBeforeDanger|WaitAfterWarps.
REDFLAGS: NoVisionMode->YouWillGetLost|ManyActionsBatched->OvershootOrStuck|WalkNorthOutBuilding->ReenterLoop|RAMDialogFlagUnreliable->ScreenshotVerify.
RATIONALIZATIONS: RAMstateEnough->LedgesFencesNPCsInvisible|FastMovementBetter->NeedVisualFeedback.
QUICKREF: Startup(server+tunnel)➔Observe(state+screenshot)➔GameLoop(orient→decide→act→verify→save)➔Stop(save+cleanup).
