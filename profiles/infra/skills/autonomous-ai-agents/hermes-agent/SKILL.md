...
This section consolidates the `provider-fallback-strategy` skill. Diagnose provider auth failures (401/403/404), check credential health, configure fallback chains, and plan budget-aware model tiering across multiple providers.
...
```
Key fields: `model.default`, `model.provider`, `fallback_providers`.

#### 2. Check Auth Status for the Failing Provider
```bash
hermes auth status <provider>
```
**"Logged in" does not mean functional** — credentials can exist but be expired, out of credits, or revoked.

#### 3. List the Full Credential Pool
```bash
hermes auth list
```
Shows ALL providers with credentials configured.

#### 4. Check If Fallback Models Use the Same Provider
Most common pitfall: `fallback_providers: []` means every fallback retries the same (dead) provider.

#### 5. Present Options to the User
| Option | Action | How |
|--------|--------|-----|
| A | Re-authenticate | `hermes auth logout <provider>` then `hermes setup` |
| B | Switch provider now | Update config.yaml `model.provider` + `model.default` |
| B2 | Route through OpenRouter | Change to `provider: openrouter` with `provider/model` format |
| C | Configure fallback providers | Add `fallback_providers` entries with different providers |
| D | Check portal | Open billing page for the provider |

### Configuring Fallback Providers

**Three-layer fallback system (priority order):**
1. `fallback_providers` — YAML list, highest priority
2. `fallback_model` — Single dict, legacy
3. `model.fallback` — Single dict, lowest priority

**Correct config format for `fallback_providers`:**
```yaml
fallback_providers:
  - provider: custom
  ...
  - provider: nous
    model: [PERSON_NAME]-coder-next
```

**CLI workflow (use `hermes fallback add` — the only reliable way):**
```bash
hermes fallback clear
hermes fallback add  # pick provider → model → confirm
hermes fallback add  # repeat for each tier
hermes fallback ls   # verify
```

### Budget-Aware Model Tiering
Configure fallback chain to maximize budget longevity:
1. **Tier 1 (default)**: Token grant providers (e.g., Xiaomi MiMo with 1B token grant)
2. **Tier 2 (fallback)**: Free rate-limited models (OpenRouter free tier)
3. **Tier 3 (fallback)**: Direct API with dollar credits (DeepSeek API)
4. **Tier 4 (fallback)**: Subscription credits (Nous Portal)

### Common Pitfalls
1. **Fallback on same provider = same failure** — always use different providers in fallback chain
2. **"Logged in" ≠ functional** — auth status shows credential exists, not that it works
3. **API key commented out in .env** — grep shows key but line starts with `#`
4. **Profile .env is isolated** — root `.env` NOT inherited by non-default profiles
5. **`hermes config set` CANNOT create YAML arrays** — use `hermes fallback add`
6. **`auxiliary.*.provider: auto` silently inherits main provider** — if main provider key dies, auxiliary calls fail

---

## Reasoning Effort Configuration

This section consolidates the `hermes-reasoning-effort-config` skill. Controls how much reasoning tokens a model generates before the final response. **Only certain providers/models support this** (Kimi/Moonshot, some LM Studio).

### Valid Values
| Value | Meaning | When to Use |
|-------|---------|-------------|
| `""` (empty) | Provider default | Recommended for most models |
| `"none"` | Disable reasoning | For models that don't support it (GPT, Claude, Llama, DeepSeek) |
...
|----------|--------|-------|
| **Kimi / Moonshot** | `moonshotai/kimi-k2`, `moonshotai/kimi-k2.6` | Native `reasoning_effort` — use `"medium"` or `"high"` |
...
OpenAI (GPT-4o, o1), Anthropic (Claude), DeepSeek, Local Llama (without LM Studio), Mistral — parameter ignored, no error.
...
| Your Model | Set `reasoning_effort` To |
|------------|---------------------------|
| Kimi / Moonshot | `"medium"` or `"high"` |
| LM Studio local | `"minimal"` or `"low"` |
| DeepSeek | `"none"` |
| OpenAI GPT | `"none"` |
| Anthropic Claude | `"none"` |
| Local Llama | `"none"` |
| Any other model | `"none"` (safe default) |
...
### Adding an In-Repo Skill

For authoring skills that ship with hermes-agent (under `skills/<category>/`), see `references/in-repo-skill-authoring.md`. Key differences from user-local skills: use `write_file` (not `skill_manage(action='create')`), always commit, and validate frontmatter before pushing.
