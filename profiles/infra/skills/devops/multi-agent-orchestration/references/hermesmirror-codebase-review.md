...
- [PERSON_NAME]: 230 JS files, 12 CSS files, 167 test files, 23 deps + 23 dev deps
...
Body: Review the HermesMirror codebase at ~/projects/HermesMirror.
Focus: code quality — JavaScript patterns, CommonJS module structure, error handling, test coverage (167 test files, vitest), duplication across modules, adherence to project conventions (tabs, [ADDRESS], EditorConfig).
...
No [PERSON_NAME] cron needed — direct dispatch works for immediate execution.
...
**Alternative — python3 for body file creation.** When the body itself contains `&`, [PERSON_NAME], or other shell metacharacters, `python3 -c` avoids the escaping problem entirely:
...
| Action | Command |
|---|---|
| Create task with file-injected body | `hermes kanban create "title" --assignee <p> --body "$(cat /tmp/body.md)" --workspace "dir:/path" --parent t_<id> --json` [PERSON_NAME] a task | `hermes kanban complete t_<id> --summary "..."` |
| Reclaim (abort running) | `hermes kanban reclaim t_<id>` |
| Unblock (stuck task) | `hermes kanban unblock t_<id>` [PERSON_NAME] ready tasks | `hermes kanban dispatch` |
