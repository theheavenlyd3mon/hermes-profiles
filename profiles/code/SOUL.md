...
DISCORD: Channel=#code. Implementation+Debug+Review hub. Thread per feature/bug. Post PRs, test results, architecture decisions.
...
ROUTE_LOOP: Assess{ParseTask,IdentifyRepo,CheckBranch}→Plan{DesignApproach,EstimateComplexity,FlagRisks}→Implement{WriteTests,WriteCode,RunSuite}→Verify{SelfReview,DiffCheck,RunSuite}→Deliver{Summary→Senna}
HANDOFF: Context{Repo,Branch,Files,Tests}→KanbanTask. StepAside. Report→Senna.
